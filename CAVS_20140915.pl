...
# * 
# * CAVS.pl is free software: you can redistribute it and/or modify
      ...
      while($line = <CAVITYFILE>)
            {
               ...
               if(($line !~ /^\#/)&&($line !~ /^END/))
                 {    
                    ...
                    open(PDBNAME, "$pdbNameFile")||die("\n\n$pdbNameFile is not available.\nExiting the program.\n\n");
...
sub AutoDockVinaHTS
{
       ...
       my($DockingResultFolder, $dockresult, $dokresultoutput);
       my($firstline, @firstlinearray, @logresultlines, $eachlogline);
       my $num = 1;
       $DockingResultFolder = "DockingResult";
       mkdir($DockingResultFolder, 0755);
       $dockresult = "docking_result";
       $dokresultoutput = $DockingResultFolder."/".$dockresult;
       open(LOGOUTFILE, ">>$dokresultoutput");
       ...
       foreach my $ligand_pdbqt_file (@ligand_name_array)
             {
                ...
                print("#                                                               #\n");
                print("#################################################################\n");
                @logresultlines = &ligandEfficiency($DockingResultFolder, $ligand_base_name, $ligand_file, $vina_output_log_path, $num_modes);
                if($num == 1)
                 {
                    $firstline = $logresultlines[0];
                    #print("this is: $firstline\n");
                    @firstlinearray = split("\t", $firstline);
                    #print("now is: $firstlinearray[0]\n");
                    $firstlinearray[0] = $ProteinPdbqtFile;
                    $firstline = join("\t", @firstlinearray);
                    #print("again is:  $firstline\n");
                    $logresultlines[0] = $firstline;
                    foreach $eachlogline (@logresultlines)
                      {
                          print LOGOUTFILE ("$eachlogline\n");
                      }
                 }
                else
                 {
                ...
                $num = $num + 1;
                #&ligandEfficiency($DockingResultFolder, $ProteinPdbqtFile, $ligand_base_name, $ligand_file, $vina_output_log_path);
             }
close(LOGOUTFILE);
closedir(LIGDIR); 
}
...
############################# *ligand Efficency calculation* #####################################################
sub ligandEfficiency
  {
      my($DckingResultFolder, $lig_base_name, $ligandname, $vinaoutput_log_path, $modenum) = @_;
      my($logfile, @logfilearray, @logarray, $linenum, @adtvinalogline, $newlogline);
      my(@dockresultarray, $ligline, $atomnum, $atom, $ligeffi);
      #$modenum = 5;
      ###### reading the ligand file to count only heavy atoms
      $atomnum = 0;
      open(LIGPDBQTFILE, "$ligandname");
      while($ligline = <LIGPDBQTFILE>)
        {
           if($ligline =~ /^HETATM/)
            {
              $atom = substr($ligline, 13, 1); 
              $atom =~ s/\s+|\s+$//g;
              if($atom ne "H")
               {
                 $atomnum = $atomnum+1;
      ...
      print("num of heavy atom:$atomnum\n");
      ###### reformating the outdock vina output and calculating ligand efficiency 
      open(LOGFILE, $vinaoutput_log_path);
      while(@logarray = <LOGFILE>)
       {
         @logfilearray = @logarray;
       }
      for($linenum = 0; $linenum<$modenum; $linenum++)
          {
            #print($logfilearray[26+$linenum]);
            $logfilearray[26+$linenum] =~ s/^\s+|\s+$//g;
            $logfilearray[26+$linenum] =~ s/\s+/\t/g;
            @adtvinalogline = split (/\s/, $logfilearray[26+$linenum]);
            $ligeffi = $adtvinalogline[1]/$atomnum;
            $ligeffi = sprintf("%.1f",  $ligeffi);
            splice(@adtvinalogline, 2, 0, $ligeffi);
            if($linenum == 0)
              {
                unshift(@adtvinalogline, "\"\"", "$lig_base_name");
                $newlogline = join("\t", @adtvinalogline);
              }
            else
              {
                unshift(@adtvinalogline, "\"\"", "\"\"");
                $newlogline = join("\t", @adtvinalogline);
              }       
            #print OUTFILE ("$newlogline\n");
            #print OUTFILE ("$logfilearray[26+$linenum]\n");
            $dockresultarray[$linenum] = $newlogline;
          }
  return(@dockresultarray);
  close(LOGFILE);
  }
